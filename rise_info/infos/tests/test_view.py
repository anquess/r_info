...
class InfoDelTest(TestCase):
    ...
    def test_info_del_count(self):
        ...
        self.assertRedirects(response, reverse('info_list'), status_code=302,
...
class AddInfoAttachmentFileTest(TestCase):
    ...
    def test_delete_info_attachment(self):
        ...
        self.assertFalse(os.path.isfile(
            f"uploads/info/{str(info_pk)}/{str(attach_pk)}/abc.jpg"))

    def test_create_info_attachment(self):
        self.assertTrue(os.path.isfile(
            f"uploads/info/{str(self.info.pk)}/{str(self.info_attach.pk)}/abc.jpg"))
        self.info_attach.file = File(open('abc.png', 'rb'))
        self.info_attach.save()
        self.assertFalse(os.path.isfile(
            f"uploads/info/{str(self.info.pk)}/{str(self.info_attach.pk)}/abc.jpg"))
        os.remove(
            f"uploads/info/{str(self.info.pk)}/{str(self.info_attach.pk)}/abc.png")
